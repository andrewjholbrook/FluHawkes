...
bench_df$V9[bench_df$V2==0] <- bench_df$V9[bench_df$V2==0] / 100
colnames(bench_df) <- c("Threads", "SIMD", "Seconds")
bench_df$Seconds <- bench_df$Seconds / 1000

bench_df$SIMD <- c("Non-vectorized","SSE",rep("AVX",10),"GPU") #rep(c("Non-vectorized","SSE",rep("AVX",10),"GPU"),2)
...
time_by_N <- read_table2("data/time_by_N.txt", 
                         col_names = FALSE)
time_by_N <- time_by_N[,-c(1,3,5,7,8)]
